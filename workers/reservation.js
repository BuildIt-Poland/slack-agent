...
exports.saveReservationAsync = async (reservationId, place, reservationParams, tableName) => (!reservationId ? await putReservation(place, reservationParams, tableName)
...
exports.listReservationsForDayAsync = async (reservation, city, tableName) => {
  ...
  if (!places) return null;
  if (!places.length) return [];
  if (!Object.keys(reservation).length) return places.map(place => ({ City: place.City, Place: place.Place, Reservation: 'free' }));
